...
async function getAllFollowedVacations(userId:number):Promise<SavedModel>{
  const sql = `
  SELECT vacation.id, destination, DATE_FORMAT(from_date, "%d-%M-%Y") AS from_date, DATE_FORMAT(to_date, "%d-%M-%Y") AS to_date, description, CONVERT(image USING utf8) as image, followers, price
                    FROM vacations.vacation
                    JOIN followers on vacation.id = followers.vacation_ID 
                    WHERE user_ID = ${userId} ORDER BY from_date
  `;
  const vacations = await dal.execute(sql);
  return vacations;
}
